...
-- Роль 'admin' для первых 5 пользователей, 'client' для следующих 20, остальные 'freelancer'
...
JOIN roles r ON (
        ...
        (u.rn > 25 AND r.name = 'ROLE_FREELANCER')
    )
WHERE u.rn <= (SELECT COUNT(*) FROM users);
